...
function c31102447.thtg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and c31102447.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c31102447.filter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectTarget(tp,c31102447.filter,tp,LOCATION_GRAVE,0,1,1,nil)
