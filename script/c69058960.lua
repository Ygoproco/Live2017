...
function c69058960.initial_effect(c)
    ...
    e4:SetCondition(c69058960.damcon)
    e4:SetOperation(c69058960.damop)
    c:RegisterEffect(e4)
end
...
function c69058960.damcon(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    return ep==tp and c:IsRelateToBattle() and eg:GetFirst()==c:GetBattleTarget() and Duel.IsExistingMatchingCard(c69058960.filter,e:GetHandlerPlayer(),LOCATION_MZONE,0,1,nil)
        and Duel.GetAttackTarget()==e:GetHandler()
end
function c69058960.damop(e,tp,eg,ep,ev,re,r,rp)
