...
function c88240808.filter(c,tp)
	return c:IsType(TYPE_MONSTER) and c:IsAbleToRemove(tp)
end
function c88240808.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(1-tp) and chkc:IsLocation(LOCATION_GRAVE) and c88240808.filter(chkc,tp) end
	if chk==0 then return Duel.IsExistingTarget(c88240808.filter,tp,0,LOCATION_GRAVE,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectTarget(tp,c88240808.filter,tp,0,LOCATION_GRAVE,1,2,nil,tp)
