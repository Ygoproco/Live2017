...
function c100419036.efffilter(c,seq,ignore_flag)
	return c:IsFaceup() and c:IsType(TYPE_EFFECT) and c:IsSetCard(0x207)
		and c:GetSequence()<5 and math.abs(c:GetSequence()-seq)<=1 and (ignore_flag or c:GetFlagEffect(100419036)==0)
end
function c100419036.effop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local seq=c:GetSequence()
	local g=Duel.GetMatchingGroup(c100419036.efffilter,tp,LOCATION_MZONE,0,nil,seq)
...
function c100419036.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return gc and gc:IsFaceup() and gc:IsLocation(LOCATION_SZONE)
		and not gc:IsDisabled() and c100419036.efffilter(e:GetHandler(),gc:GetSequence(),true)
