...
function c95442074.initial_effect(c)
    ...
    e4:SetCondition(c95442074.damcon)
    e4:SetOperation(c95442074.damop)
    c:RegisterEffect(e4)
end
...
function c95442074.damcon(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    return ep==tp and c:IsRelateToBattle() and eg:GetFirst()==c:GetBattleTarget() and Duel.IsExistingMatchingCard(c95442074.filter,e:GetHandlerPlayer(),LOCATION_MZONE,0,1,nil)
        and Duel.GetAttackTarget()==e:GetHandler()
end
function c95442074.damop(e,tp,eg,ep,ev,re,r,rp)
