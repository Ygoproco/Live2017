...
function c10000080.chk(c)
	return c:IsCode(90846359,53334471) and not c:IsStatus(STATUS_DISABLED)
end
function c10000080.rivgoz(c,tc,g)
	if g:IsExists(Card.IsCode,1,nil,90846359) then return c:IsRace(tc:GetRace()) end
	if g:IsExists(Card.IsCode,1,nil,53334471) then return c:IsAttribute(tc:GetAttribute()) end
end
function c10000080.ttcon2(e,c,minc)
	if c==nil then return true end
	local tp=c:GetControler()
	if Duel.IsExistingMatchingCard(c10000080.chk,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) then
		local g=Duel.GetMatchingGroup(c10000080.chk,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil)
		local mg=Duel.GetMatchingGroup(c10000080.rivgoz,tp,0,LOCATION_MZONE,nil,c,g)
