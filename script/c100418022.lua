...
function c100418022.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(100418022,0))
	e1:SetCategory(CATEGORY_DRAW)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e1:SetCode(EVENT_TO_GRAVE)
	e1:SetCondition(c100418022.con)
	e1:SetTarget(c100418022.tg)
	e1:SetOperation(c100418022.op)
	c:RegisterEffect(e1)
	--search
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(100418022,1))
	e2:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_HAND)
	e2:SetCountLimit(1,100418022)
	e2:SetCost(c100418022.cost)
	e2:SetTarget(c100418022.target)
	e2:SetOperation(c100418022.operation)
	c:RegisterEffect(e2)
	--send to grave
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(100418022,2))
	e3:SetCategory(CATEGORY_TOGRAVE)
	e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_PRE_DAMAGE_CALCULATE)
	e3:SetRange(LOCATION_SZONE)
	e3:SetCountLimit(1,100418022+100)
	e3:SetCondition(c100418022.gycon)
	e3:SetTarget(c100418022.gytg)
	e3:SetOperation(c100418022.gyop)
	c:RegisterEffect(e3)
end
