...
function c6983839.filter(c)
	return c:IsType(TYPE_SPELL+TYPE_TRAP)
end
function c6983839.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and c6983839.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c6983839.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c6983839.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
