...
function c26211048.eqfilter(c)
	return (c:IsLocation(LOCATION_MZONE) or c:IsLocation(LOCATION_GRAVE)) and c:IsType(TYPE_MONSTER) and not c:IsForbidden()
end
function c26211048.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return c26211048.eqfilter(chkc) and chkc:IsControler(1-tp) end
