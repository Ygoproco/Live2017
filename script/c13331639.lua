...
function c13331639.fusfilter1(c)
	...
	return c:IsHasEffect(511002961) or (c:IsRace(RACE_DRAGON) and c:IsFusionType(TYPE_FUSION))
end
function c13331639.fusfilter2(c)
	if c:IsHasEffect(6205579) then return false end
	return c:IsHasEffect(511002961) or (c:IsRace(RACE_DRAGON) and c:IsFusionType(TYPE_SYNCHRO))
end
function c13331639.fusfilter3(c)
	if c:IsHasEffect(6205579) then return false end
	return c:IsHasEffect(511002961) or (c:IsRace(RACE_DRAGON) and c:IsFusionType(TYPE_XYZ))
end
function c13331639.fusfilter4(c)
	if c:IsHasEffect(6205579) then return false end
	return c:IsHasEffect(511002961) or (c:IsRace(RACE_DRAGON) and c:IsFusionType(TYPE_PENDULUM))
end
...
function c13331639.FConditionFilterMulti(c,mg,funs,n,tbt,g,p)
	for i=1,n do
		local tp=2^(i-1)
		if bit.band(tbt,tp)~=0 and funs[i](c) then
			...
			local g2=g:Clone()
			g2:AddCard(c)
			if t2==0 then return not g2:IsExists(aux.TuneMagFusFilter,1,nil,g2,p) end
			local mg2=mg:Clone()
			mg2:RemoveCard(c)
			if mg2:IsExists(c13331639.FConditionFilterMulti,1,nil,mg2,funs,n,t2,g2,p) then return true end
		end
	end
	return false
end
...
function c13331639.FConditionFilterMulti2(c,gr,chkf,g,tp)
	...
	table.remove(gr2,1)
	if #gr2==0 then
		if mg:IsExists(aux.TuneMagFusFilter,1,nil,mg,tp) then return false end
		return chkf==PLAYER_NONE or mg:IsExists(aux.FConditionCheckF,1,nil,chkf)
	else
		return gr2[1]:IsExists(c13331639.FConditionFilterMulti2,1,nil,gr2,chkf,mg,tp)
	end
end
function c13331639.FConditionFilterMultiSelect(c,funs,n,mg,sg,tp)
	local valid=c13331639.FConditionFilterMultiValid(sg,funs,n)
	if not valid then valid={0} end 
	local all = (2^n)-1
	for k,v in pairs(valid) do
		v=bit.bxor(all,v)
		if c13331639.FConditionFilterMulti(c,mg,funs,n,v,sg,tp) then return true end
	end
	return false
end
...
function c13331639.fuscon(e,g,gc,chkfnf)
	...
	if gc then
		...
		for i=1,n do
			if funs[i](gc) then
				...
				if mg2:IsExists(c13331639.FConditionFilterMulti,1,nil,mg2,funs,n,tbt,Group.FromCards(gc),tp) then return true end
			end
		end
		return false
	end
	...
	return gr2[1]:IsExists(c13331639.FConditionFilterMulti2,1,nil,gr2,chkf,Group.CreateGroup(),tp)
end
function c13331639.fusop(e,tp,eg,ep,ev,re,r,rp,gc,chkfnf)
	...
	if gc then
		...
		mg:RemoveCard(gc)
		for i=1,n-1 do
			local mg2=mg:Filter(c13331639.FConditionFilterMultiSelect,nil,funs,n,mg,sg,tp)
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			local sg2=mg2:Select(p,1,1,nil)
	...
	for i=1,n do
		...
		if i==1 and chkf~=PLAYER_NONE then
			sg2=mg2:FilterSelect(p,aux.FConditionCheckF,1,1,nil,chkf)
		else
			sg2=mg2:Select(p,1,1,nil)
		end
		sg:AddCard(sg2:GetFirst())
		mg:RemoveCard(sg2:GetFirst())
	end
...
function c13331639.check_fusion_material_48144509(g,chkf)
	...
	local gr2=c13331639.CloneTable(groups)
	return gr2[1]:IsExists(c13331639.FConditionFilterMulti2,1,nil,gr2,chkf,Group.CreateGroup(),chkf)
end
function c13331639.select_fusion_material_48144509(tp,g,chkf)
	...
	local funs=c13331639.filters
	for i=1,4 do
		local mg2=mg:Filter(c13331639.FConditionFilterMultiSelect,nil,funs,4,mg,sg,tp)
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
		local sg2=nil
		if i==1 and chkf~=PLAYER_NONE then
