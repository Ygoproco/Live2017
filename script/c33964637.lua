...
function c33964637.initial_effect(c)
	...
	e1:SetCondition(c33964637.fscon)
	e1:SetOperation(c33964637.fsop)
	c:RegisterEffect(e1)
	--special summon rule
...
function c33964637.fselect(c,mg,sg,tp,fc)
	...
	if sg:GetCount()==3 then
		...
		res=mg:IsExists(c33964637.fselect,1,sg,mg,sg,tp,fc)
	end
	sg:RemoveCard(c)
	return res
end
function c33964637.fscon(e,g,gc,chkf)
	if g==nil then return true end
	local c=e:GetHandler()
	local tp=c:GetControler()
	local mg=g:Filter(c33964637.ffilter,nil,c)
	local sg=Group.CreateGroup()
	if gc then return c33964637.ffilter(gc,c) and c33964637.fselect(gc,mg,sg,tp,c) end
	return mg:IsExists(c33964637.fselect,1,sg,mg,sg,tp,c)
end
function c33964637.fsop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	local c=e:GetHandler()
	local mg=eg:Filter(c33964637.ffilter,nil,c)
	local sg=Group.CreateGroup()
	if gc then sg:AddCard(gc) end
	repeat
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
		local g=mg:FilterSelect(tp,c33964637.fselect,1,1,sg,mg,sg,tp,c)
		sg:Merge(g)
	until sg:GetCount()==3
...
function c33964637.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	return Duel.SelectYesNo(tp,aux.Stringid(33964637,1))
end
